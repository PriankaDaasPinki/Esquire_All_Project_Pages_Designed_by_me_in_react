...
const FinancialYearListComponent = ({
  apiEndpoint,
  detailsURL,
  ...
  const { data, totalItems, isLoading, error, fetchData } = useFetchListData(
    `${apiEndpoint}`,
    params
  );
  ...
  const dataSource = data?.flatMap((data, index) => ({
    ...
    status:
      data?.status === false ? (
        <div className="bg-red-500 text-[#fff] w-[70px] text-center rounded-[10px] p-[1px]">
          Inactive
        </div>
      ) : (
        <div className="bg-green-500 text-[#fff] w-[70px] text-center rounded-[10px] p-[1px]">
          Active
        </div>
      ),
    is_close:
    ...
    if (result.isConfirmed) {
      try {
        const response = await authAxios.delete(
          `${baseURL}drf-finance/financial-years/${id}/`
        );
